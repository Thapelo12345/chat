...
io.on("connection", (socket) => {
  ...
  socket.on("confirmation", (confirm, theOneWhoAksed) => {
    ...
    if (confirm === "yes") {
      singleChatPairs.push({ user1: getUser1, user2: getUser2 });
    }

    socket
      .to(userNameAndId.find((item) => item.name === theOneWhoAksed).id)
      .emit("final-response", confirm, session.userName, ConfirmingUserId);
  }); //end of confirmation socket

  socket.on("send-private-message", (msg) => {
    let person_who_sent_msg = userNameAndId.find(
      (item) => item.id === socket.id
    ).name;

    let sendingTo;
    let pair = singleChatPairs.find(
      (item) =>
        item.user1 === person_who_sent_msg || item.user2 === person_who_sent_msg
    );
    pair.user1 === person_who_sent_msg
      ? (sendingTo = pair.user2)
      : (sendingTo = pair.user1);

    socket
      .to(userNameAndId.find((item) => item.name === sendingTo).id)
      .emit("recieve-private-message", msg);
  }); //recieving a private message

  socket.on("close-private-chat", () => {
    let userClosingChat = session.userName;
    let get_the_pair = singleChatPairs.find(
      (item) => item.user1 === userClosingChat || item.user2 === userClosingChat
    );

    let sendingTo;

    userClosingChat !== get_the_pair.user1
      ? (sendingTo = get_the_pair.user1)
      : (sendingTo = get_the_pair.user2);
    socket
      .to(userNameAndId.find((item) => item.name === sendingTo).id)
      .emit("close-chat-notification", userClosingChat);
  ...
  socket.on("join_group", (group) => {
    ...
    if (group && io.sockets.adapter.rooms.get(group)) {
      ...
      socket
        .to(group)
        .emit("update_count", io.sockets.adapter.rooms.get(group).size);
    } //end of if
  }); //end of join group socket

  socket.on("sending-group-notification", (group) => {
    let getName =
      userNameAndId.find((item) => item.id === socket.id) !== undefined
        ? userNameAndId.find((item) => item.id === socket.id).name
        : "Someone";
    let usersInGroup = userAndGroup.filter(
...
app.get("/data", async (req, res) => {
  try {
    ...
    if (!personToFind) {
      personToFind = { username: req.session.userName };
    }
    ...
    const pic =
      personToFind.profilePicture &&
      personToFind.profilePicture.data &&
      personToFind.profilePicture.data.length > 0
        ? "yes"
        : "no";
...
app.get("/get_profile_pic:userId", async (req, res) => {
  try {
    ...
    if (user.profilePicture.data && user.profilePicture.data.length > 0) {
      ...
      res.status(204).send("User has no Pic");
    } //end of else
  } catch (err) {
    res.send(err);
  }
...
app.post("/login", async (req, res) => {
  ...
  if (user) {
    ...
    res.json({ message: `The is no use with the name : ${req.body.username}` });
  }
}); //end of login
...
app.post("/create/group", (req, res) => {
  Group.findOne({ groupName: req.body.group })
    .then((group) => {
      if (group) {
        res.json({ message: "Group name already exists!" });
      } //end of if
      else {
        ...
        newGroup
          .save()
          .then(() => {
            "Group create and svae successfuly!";
          })
...
app.post("/search/user", (req, res) => {
  let nameToSearch = req.body.findMe,
    modelToFind = req.body.userOrGroup;
  ...
  if (modelToFind === "user") {
    ...
    Group.findOne({ groupName: nameToSearch })
      .then((group) => {
        if (group) {
          res.json({ message: group.groupName });
        } else {
